...
with no `name` either, a default user icon shows. Set `colorful={false}` for a
neutral grey tint instead of the auto colour.
...
| `icon` | `ReactNode` | user glyph | Shown when there's no image and no name. |
