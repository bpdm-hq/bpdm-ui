...
Why the bar exists: a subtle brand tint alone sits at ~1.05:1 against the panel — far below the WCAG 1.4.11 **≥3:1** an active-state indicator needs. The bar (built on `--ring`, engineered ≥3:1 on light) is what actually carries that contrast, so a low-vision or keyboard user can always tell which option is focused. The tint is decorative on top.
