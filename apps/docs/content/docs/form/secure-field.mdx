...
- The control labels (`Reveal` / `Hide` / `Copy`) and the copy announcement (`Copied to
  clipboard`) are translatable via the `messages` prop — see
  [Translating labels](/docs/getting-started/i18n). Give the field a translated accessible name.
- **RTL-safe**: no physical layout (the controls sit via flex), so the field mirrors under
  `dir="rtl"`.
