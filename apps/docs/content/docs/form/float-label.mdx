...
- The label is a real `<label>` associated with the control by `for`/`id`, so clicking it focuses
  the field and assistive tech announces it as the field's **name**. The association is deterministic:
  the id comes from `htmlFor`, else the control's own `id`, else a generated one — so it never dangles.
...
- The floating `label` is **your** text — pass it already translated (same for any `placeholder`
  on the wrapped control). There's nothing hard-coded to override.
- **RTL-safe**: the resting label position uses a logical property (`start-3`), so the label rests
  and floats from the correct side under `dir="rtl"`. See the
