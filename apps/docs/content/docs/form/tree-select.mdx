...
- A real **WAI-ARIA tree**: the panel is `role="tree"` (`aria-multiselectable`), each node is a
  `role="treeitem"` carrying `aria-level`, `aria-expanded` (branches), and `aria-checked`
  (`true` / `false` / **`mixed`** — a parent is mixed when only some descendants are selected);
  child nodes sit in a nested `role="group"`. The trigger is `role="combobox"` with
  `aria-haspopup="tree"` and `aria-controls`.
