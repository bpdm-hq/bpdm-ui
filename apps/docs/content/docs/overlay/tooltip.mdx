...
- The bubble is **hoverable**: moving the pointer off the trigger and onto the tooltip
  itself keeps it open, so you can read (or reach content in) a tooltip without it
  vanishing — in both React and Angular.
