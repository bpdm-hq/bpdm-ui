...
| --- | --- | --- |
| `ariaLabel` | `Progress` | Accessible name for the step list (`tablist`). |
...
Both frameworks implement the same **tablist** pattern for the steps:

- **Roles and wiring.** The step list is a `role="tablist"` with an `aria-label` (from
  `messages.ariaLabel`) and `aria-orientation` matching `orientation`. Each step is a
  `role="tab"` carrying `aria-selected`, and the active step also carries
  `aria-current="step"`. A step is linked to its panel both ways — `aria-controls` from
  the step to the `role="tabpanel"`, and `aria-labelledby` from the panel back to the
  step.
...
- **Keyboard.** **Tab** moves between the steps (each reachable step is a tab stop),
  and **Enter** / **Space** activates a reachable step. The **Back** / **Next** /
