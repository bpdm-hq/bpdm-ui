...
- **`dismiss`** — the close (X) button's accessible name (`aria-label`). Default
  `"Dismiss"`.
- **`regionLabel`** — the accessible label for the toast live-region / viewport
  landmark (React's Radix `Toast.Provider`/`Toast.Viewport` label and Angular's
  `role="region"` list). Default `"Notifications"`.
