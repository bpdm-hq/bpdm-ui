...
`<div>` attributes are forwarded onto the root.

## Accessibility

- The alert container has **`role="alert"`**, so assistive tech announces it when
  it appears — use it for important, in-context messages.
- The dismiss button has an `aria-label="Dismiss"` and is keyboard focusable; the
  leading icon is decorative (`aria-hidden`), so meaning comes from the text.
- Don't rely on colour alone — the `title`/body should read clearly on their own
  (the per-variant icon reinforces, but doesn't replace, the wording).
