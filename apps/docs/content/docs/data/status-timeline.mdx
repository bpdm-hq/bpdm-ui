          ...
          if (statusOf(id) === 'current')
            return (
              <button onClick={() => complete(id)} aria-label={`Complete step ${id}`}
                className="grid size-7 place-items-center rounded-full bg-primary text-primary-foreground ring-4 ring-primary/20 transition-transform hover:scale-110">
                {id}
              </button>
            );
      ...
      <ng-template #marker let-item>
        @if (statusOf(item.id) === 'current') {
          <button (click)="complete(item.id)" [attr.aria-label]="'Complete step ' + item.id"
            class="grid size-7 place-items-center rounded-full bg-primary text-primary-foreground ring-4 ring-primary/20 transition-transform hover:scale-110">
            {{ item.id }}
          </button>
        } @else {
