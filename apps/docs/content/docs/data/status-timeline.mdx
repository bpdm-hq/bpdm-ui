...
its state (see [Accessibility](#accessibility)). Those four strings — and only those
...
```tsx title="status-timeline-i18n.tsx"
...
```ts title="status-timeline-i18n.ts"
import { Component } from '@angular/core';
...
The visually-hidden status text announced for each step (screen readers), merged
over the English defaults — override any subset.
...
- Status is **never conveyed by colour or glyph alone**: the marker (✓ complete, a
  filled pulsing dot for current, a hollow ring for pending, ✗ failed) is decorative
  (`aria-hidden`), so each step also renders a **visually-hidden status label** —
  "Completed" / "In progress" / "Not started" / "Failed" — that screen-reader users
  hear alongside the `title`. Those four strings are translatable via `messages` (see
...
- Layout uses **logical CSS properties**, so the timeline mirrors correctly under
