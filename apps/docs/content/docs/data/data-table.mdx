...
export function BorderTables() {
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* zebra rows */}
      <DataTable columns={columns} data={members} rowKey={(r) => r.id} striped />
      {/* outlined card with column gridlines */}
      <DataTable columns={columns} data={members} rowKey={(r) => r.id} frame bordered />
    </div>
  );
}
```
...
@Component({
  ...
  template: `
    <div class="flex flex-col gap-6">
      <bpdm-data-table [columns]="columns" [data]="members" [rowKey]="rowKey" />
      <bpdm-data-table [columns]="columns" [data]="members" [rowKey]="rowKey" striped />
      <bpdm-data-table [columns]="columns" [data]="members" [rowKey]="rowKey" frame bordered />
    </div>
  `,
})
