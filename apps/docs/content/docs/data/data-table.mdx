...
// a rich identity cell: avatar + name + secondary line
...
export class MembersTable {
  ...
  readonly initials = (name: string) => name.split(' ').map((p) => p[0]).slice(0, 2).join('');
...
<ng-template #memberCell let-row>
  <div class="flex items-center gap-3">
    <span class="grid size-9 place-items-center rounded-full bg-muted text-xs font-semibold">
      {{ initials(row.name) }}
    </span>
