...
scrolling horizontally. Everything else still works — **sorting**, **filtering**, and
rich custom cells (an avatar, an avatar group, a flag, a row-actions column) all
carry through; on narrow screens they simply become the card's values. **Resize the
preview** to see it switch.
...
export class ResponsiveTable {
  ...
  readonly columns = computed<DataTableColumn<Member>[]>(() => [
    { id: 'name', header: 'Name', sortable: true, filterable: true, sortAccessor: (r) => r.name, cell: this.nameCell() },
    ...
    { id: 'role', header: 'Role', sortable: true, filterable: true, filterType: 'select', accessor: (r) => r.role },
    { id: 'seats', header: 'Seats', numeric: true, sortable: true, accessor: (r) => r.seats },
    { id: 'actions', header: 'Actions', align: 'right', hideable: false, cell: this.actionsCell() },
  ]);
}
```

```html title="data-table-responsive.html"
<bpdm-data-table [columns]="columns()" [data]="members" [rowKey]="rowKey" responsive />
