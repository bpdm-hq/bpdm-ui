...
## Rich cells & row actions

Because every `cell` is a renderer, columns can hold anything — an **avatar group**
of collaborators, a **flag + region**, a numeric metric, a status [`Badge`](/docs/messages/badge),
or a trailing **action column** of icon [`Button`](/docs/form/button)s (edit, configure,
remove). A small, production-grade composition:
...
```tsx title="data-table-actions.tsx"
import { DataTable, type DataTableColumn } from '@bpdm/ui/data-table';
...
const members: Member[] = [
  { id: 'm1', name: 'Hugo Lindberg', flag: '🇸🇪', country: 'Sweden', seats: 24, status: 'active', team: ['Sara Kovac', 'Ivan Petrov', 'Ava Nguyen'] },
  { id: 'm2', name: 'Leo Martins', flag: '🇧🇷', country: 'Brazil', seats: 8, status: 'invited', team: ['Hugo Lindberg', 'Noah Bauer'] },
  { id: 'm3', name: 'Sara Kovac', flag: '🇭🇷', country: 'Croatia', seats: 16, status: 'active', team: ['Ava Nguyen', 'Ivan Petrov'] },
];

const STATUS = { active: 'success', invited: 'warning', disabled: 'neutral' } as const;

// any SVG / icon library works — a tiny inline helper here
...
const columns: DataTableColumn<Member>[] = [
  ...
  { id: 'seats', header: 'Seats', numeric: true, accessor: (r) => r.seats },
  {
    ...
    align: 'center',
    cell: (r) => (
      <Badge variant={STATUS[r.status]} appearance="soft" dot className="capitalize">
        {r.status}
      </Badge>
    ),
  },
  {
    ...
    cell: (r) => (
      <div className="flex justify-end gap-0.5">
        ...
        <Button variant="secondary" appearance="ghost" size="iconSm" aria-label={`Configure ${r.name}`}>
          <Icon d="M8 10a2 2 0 1 0 0-4 2 2 0 0 0 0 4ZM8 2v1.5M8 12.5V14M3.5 3.5l1 1M11.5 11.5l1 1M2 8h1.5M12.5 8H14M3.5 12.5l1-1M11.5 4.5l1-1" />
        </Button>
...
Cells are `<ng-template>`s; the avatar group is `<bpdm-avatar-group [users]>` and the
actions are `button[bpdmButton]` icon buttons.

```ts title="data-table-actions.ts"
import { Component, viewChild, computed, type TemplateRef } from '@angular/core';
import { BpdmDataTable, BpdmAvatar, BpdmAvatarGroup, BpdmBadge, BpdmButton, type DataTableColumn } from '@bpdm/ng';
...
@Component({
  selector: 'row-actions-table',
  imports: [BpdmDataTable, BpdmAvatar, BpdmAvatarGroup, BpdmBadge, BpdmButton],
  templateUrl: './data-table-actions.html',
})
export class RowActionsTable {
  readonly memberCell = viewChild.required<TemplateRef<{ $implicit: Member }>>('memberCell');
  ...
  readonly rowKey = (r: Member) => r.id;
  readonly statusVariant = (s: Member['status']) => ({ active: 'success', invited: 'warning', disabled: 'neutral' } as const)[s];
  readonly members: Member[] = [
    { id: 'm1', name: 'Hugo Lindberg', flag: '🇸🇪', country: 'Sweden', seats: 24, status: 'active', team: [{ name: 'Sara Kovac' }, { name: 'Ivan Petrov' }, { name: 'Ava Nguyen' }] },
    { id: 'm2', name: 'Leo Martins', flag: '🇧🇷', country: 'Brazil', seats: 8, status: 'invited', team: [{ name: 'Hugo Lindberg' }, { name: 'Noah Bauer' }] },
    { id: 'm3', name: 'Sara Kovac', flag: '🇭🇷', country: 'Croatia', seats: 16, status: 'active', team: [{ name: 'Ava Nguyen' }, { name: 'Ivan Petrov' }] },
  ];

  readonly columns = computed<DataTableColumn<Member>[]>(() => [
    { id: 'member', header: 'Member', cell: this.memberCell() },
    { id: 'team', header: 'Collaborators', cell: this.teamCell() },
    { id: 'location', header: 'Location', cell: this.locationCell() },
    { id: 'seats', header: 'Seats', numeric: true, accessor: (r) => r.seats },
    { id: 'status', header: 'Status', align: 'center', cell: this.statusCell() },
    { id: 'actions', header: '', align: 'right', cell: this.actionsCell() },
  ]);
}
```

```html title="data-table-actions.html"
<bpdm-data-table [columns]="columns()" [data]="members" [rowKey]="rowKey" />
...
<ng-template #statusCell let-row>
  <bpdm-badge [variant]="statusVariant(row.status)" appearance="soft" dot class="capitalize">{{ row.status }}</bpdm-badge>
</ng-template>
...
<ng-template #actionsCell let-row>
  <div class="flex justify-end gap-0.5">
    ...
    <button bpdmButton variant="secondary" appearance="ghost" size="iconSm" [attr.aria-label]="'Configure ' + row.name"><!-- gear icon --></button>
