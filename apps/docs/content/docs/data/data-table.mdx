...
a header to append it to the sort (each shows its order). The preview toggles the two
modes with our `Tabs`; in **Multi sort** rows are grouped by **role**, then by **tasks**
within each role. Leave `sort` unset to let the table sort internally, or control
`sort` + `onSortChange` for server-side ordering.
