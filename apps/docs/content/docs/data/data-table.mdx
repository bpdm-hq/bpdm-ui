...
// Client-side (default) — the table filters `data` in memory
export function FilterableTable() {
  return <DataTable columns={columns} data={members} rowKey={(r) => r.id} />;
}

// Server-side — control `filters`; the same funnel UI shows, but you fetch the
// filtered rows. `ColumnFilter` = { matchMode: 'all' | 'any', rules: [{ op, value }] }.
export function ServerFilteredTable() {
  const [filters, setFilters] = useState<Record<string, ColumnFilter>>({});
  const rows = useFilteredMembers(filters); // your data hook — re-fetches on change
  return (
...
@Component({
  ...
  template: `
    ...
    <!-- Server-side — control [filters]; re-fetch on (filtersChange) -->
    <bpdm-data-table
      [columns]="columns" [data]="serverRows()" [rowKey]="rowKey"
...
export class FilterableTable {
  ...
  // server-side: filter state + the rows your backend returns
...
// Client-side (default) — the table searches `data` in memory
...
// Server-side — control `searchValue`; the box is shown but you fetch the results
export function ServerSearchTable() {
  const [q, setQ] = useState('');
  const rows = useSearchedMembers(q); // your data hook — re-fetches on change (debounce it!)
  return (
    <DataTable
      columns={columns}
      data={rows}
...
@Component({
  ...
  template: `
    <!-- Client-side (default) -->
    <bpdm-data-table [columns]="columns" [data]="members" [rowKey]="rowKey" searchable searchPlaceholder="Search members…" />

    <!-- Server-side — control [searchValue]; re-fetch on (searchChange), debounce it -->
    <bpdm-data-table
      [columns]="columns" [data]="serverRows()" [rowKey]="rowKey"
      searchable [searchValue]="q()" (searchChange)="onSearch($event)" />
  `,
})
export class SearchableTable {
  ...
  // server-side: query + the rows your backend returns
  readonly q = signal('');
  readonly serverRows = signal<Member[]>([]);
  onSearch(value: string) {
    this.q.set(value);
    // re-fetch (debounced): this.api.search(value).subscribe(rows => this.serverRows.set(rows));
  }
}
```
