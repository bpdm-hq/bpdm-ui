...
- Selection checkboxes/radios are labelled; **select-all** is a labelled header
  checkbox with an indeterminate state. Pagination controls are labelled buttons
  and the current page carries **`aria-current`**. Decorative icons are `aria-hidden`.
