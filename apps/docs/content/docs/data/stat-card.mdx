...
- The `label` and `value` are plain text, announced in reading order.
- The delta never relies on colour alone — an **arrow + sign** convey direction and
  `deltaLabel` gives it context, so it reads clearly without perceiving red/green.
