...
- The coloured delta **text** uses the darkened `-strong` colour tokens, so red
  (negative) and green (positive) deltas measure at **≥4.5:1** against the card on
  the light themes (paper, mist) and stay unchanged on the dark themes (charcoal,
  slate). The arrow glyph is decorative and inherits the same colour.
