...
- The coloured delta **text** uses the darkened `-strong` colour tokens, so red
  (negative) and green (positive) deltas clear the **4.5:1** text-contrast bar
  (WCAG 1.4.3) on the light themes (paper, mist) and stay unchanged on the dark
  themes (charcoal, slate). The arrow glyph is decorative and inherits the same colour.
