...
@Component({
  ...
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: { role: "tablist", class: "flex items-center" },
  template: `<ng-content />`,
})
  ...
  template: `
    <div class="relative" [class.pb-2]="!isLast()">
      @if (!isLast()) {
        <span
          aria-hidden="true"
          class="absolute bottom-0 left-4 top-9 w-0.5 -translate-x-1/2 overflow-hidden rounded-full bg-border"
        >
          <span
    ...
    <div [class]="vertical() ? 'contents' : isLast() ? 'flex items-center' : 'flex flex-1 items-center'">
      ...
      @if (!vertical() && !isLast()) {
        <span aria-hidden="true" class="relative mx-3 h-0.5 flex-1 overflow-hidden rounded-full bg-border">
          <span
            class="absolute inset-0 origin-left rounded-full bg-primary transition-transform duration-[360ms] ease-[var(--bpdm-ease-out)]"
...
export class BpdmStep {
  ...
  protected readonly markerClass = computed(() =>
    cn(
      ...
      this.state() === "active" && "scale-105 border-primary text-primary",
...
@Component({
  ...
  template: `
    <div role="tabpanel" [attr.aria-hidden]="!active()" [class]="outerClass()">
...
export class BpdmStepPanel {
  ...
  protected readonly innerClass = computed(() =>
    cn("text-sm text-foreground", this.vertical() ? "ml-[2.75rem] pb-2 pt-1" : "pt-2"),
  );
}
