...
export class FaqComponent {
  ...
  private rocket = viewChild<TemplateRef<unknown>>('rocket');
  private rotate = viewChild<TemplateRef<unknown>>('rotate');
  private file = viewChild<TemplateRef<unknown>>('file');
  private users = viewChild<TemplateRef<unknown>>('users');
  items = computed<AccordionItemData[]>(() => {
    const d = this.deploys(), r = this.rollback(), l = this.logs(), m = this.members();
    const ri = this.rocket(), ro = this.rotate(), fi = this.file(), us = this.users();
    return d && r && l && m && ri && ro && fi && us ? [
      { value: 'deploys', title: 'How are deploys triggered?', content: d, icon: ri },
      { value: 'rollback', title: 'Can I roll back a release?', content: r, icon: ro },
      { value: 'logs', title: 'Where are build logs stored?', content: l, icon: fi },
      { value: 'members', title: 'How do I invite a teammate?', content: m, icon: us },
    ] : [];
  });
}`,
      },
